  ...
  return (
    <div className="space-y-6">
      <div className="glass rounded-3xl p-6">
        ...
        <p className="text-sm text-ink-muted">Live configuration snapshot for production operations.</p>
      </div>
      ...
      <div className="card-outline rounded-3xl bg-surface-card p-6">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-accent/10 bg-surface-card px-4 py-3 text-sm">
            <p className="text-xs uppercase tracking-[0.2em] text-ink-muted">Commission rates</p>
            <p className="mt-2 font-semibold text-ink">Level 1: {((commissions[1] || 0) * 100).toFixed(1)}%</p>
            <p className="mt-1 text-xs text-ink-muted">Level 2: {((commissions[2] || 0) * 100).toFixed(1)}%</p>
            <p className="mt-1 text-xs text-ink-muted">Level 3: {((commissions[3] || 0) * 100).toFixed(1)}%</p>
          </div>
          <div className="rounded-2xl border border-accent/10 bg-surface-card px-4 py-3 text-sm">
            <p className="text-xs uppercase tracking-[0.2em] text-ink-muted">Payments</p>
            <p className="mt-2 font-semibold text-ink">
              Paystack status: {settings?.paystackConfigured ? "Configured" : "Missing keys"}
            </p>
            <p className="mt-1 text-xs text-ink-muted">Base URL: {settings?.baseUrl || "Not set"}</p>
          </div>
          <div className="rounded-2xl border border-accent/10 bg-surface-card px-4 py-3 text-sm">
            <p className="text-xs uppercase tracking-[0.2em] text-ink-muted">AFA registration fee</p>
            ...
            <form action={updateAfaFee} className="mt-3 flex flex-wrap items-center gap-2">
              <input
                ...
                min="1"
                step="0.01"
                className="w-full rounded-2xl border border-accent/10 bg-surface-card px-3 py-2 text-xs md:w-auto"
          ...
          <div className="rounded-2xl border border-accent/10 bg-surface-card px-4 py-3 text-sm">
            <p className="text-xs uppercase tracking-[0.2em] text-ink-muted">Data provider</p>
              ...
              <button
                className={`w-full rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] ${
                  dispatcherEnabled
                    ...
                    : "bg-accent text-night hover:bg-accent"
                }`}
              >
                {dispatcherEnabled ? "Turn dispatcher off" : "Turn dispatcher on"}
              </button>
            </form>
            <div className="mt-4 rounded-2xl bg-surface-elevated px-4 py-3 text-xs">
              <p className="text-[0.65rem] uppercase tracking-[0.25em] text-ink-muted">GrandAPI balance</p>
              {providerBalance ? (
                <div className="mt-2 space-y-1 font-mono text-sm text-ink">
                  ...
                  <p className="text-ink-muted">Total sales: {(providerBalance.totalSales / 100).toFixed(2)} GHS</p>
                  <p className="text-ink-muted">Total deposit: {(providerBalance.totalDeposit / 100).toFixed(2)} GHS</p>
                  <p className="text-ink-muted">Overdraft: {(providerBalance.overDraft / 100).toFixed(2)} GHS</p>
                </div>
              ) : (
                <p className="mt-2 text-ink-muted">Balance unavailable</p>
              )}
              <form action={refreshProviderBalance} className="mt-3">
                <button className="w-full rounded-full border border-accent/15 px-4 py-2 text-[0.65rem] font-semibold uppercase tracking-[0.3em] text-ink">
                  Refresh balance
                </button>
              </form>
            </div>
