  ...
  return (
    <div className="space-y-6">
      <div className="glass rounded-3xl p-6">
        ...
        <p className="text-sm text-ink-muted">Track Paystack references and webhook status.</p>
      </div>
      ...
      <div className="card-outline rounded-3xl bg-surface-card p-6">
        <div className="space-y-3">
          {payments.length === 0 ? (
            ...
            payments.map((item) => (
              <div key={item.id || item.reference} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-accent/10 bg-surface-card px-4 py-3 text-sm">
                <div>
