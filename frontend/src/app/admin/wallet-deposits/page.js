  ...
  return (
    <div className="space-y-6">
      <div className="glass rounded-3xl p-6">
        ...
        <p className="text-sm text-ink/60">Review all successful wallet top-ups across agents.</p>
      </div>
            ...
            deposits.map((item) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-ink/60">{item.wallet?.agent?.email || "No email"}</p>
                  <p className="text-xs text-ink/60">Ref: {item.reference || "-"}</p>
                </div>
