  ...
  return (
    <div className="space-y-6">
      <div className="glass rounded-3xl p-6">
        ...
        <p className="text-sm text-ink-muted">Approve or reject agent API access requests.</p>
      </div>
            ...
            requests.map((item) => (
              <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-accent/10 bg-surface-card px-4 py-3 text-sm">
                <div>
                  ...
                  <p className="text-xs text-ink-muted">{item.agent?.email || ""}</p>
                  <p className="text-xs text-ink-muted">Requested: {new Date(item.createdAt).toLocaleDateString()}</p>
                </div>
