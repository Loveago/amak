...
export default async function AgentApiPage({ searchParams = {} }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card-outline rounded-3xl bg-surface-card p-6">
        ...
        {isRejected && (
          <div className="mt-4 rounded-2xl border border-red-500/20 bg-red-500/5 px-4 py-3">
            ...
            <p className="text-xs text-red-400">Your API access request was rejected. Contact support for more information.</p>
          </div>
        )}

        {isApproved && (
          <div className="mt-4 rounded-2xl border border-accent/20 bg-accent/10 px-4 py-3">
            <p className="text-sm font-semibold text-accent">Approved</p>
            <p className="text-xs text-accent">Your API access is active. You can generate an API key below.</p>
          </div>
        )}
      </div>
      ...
      {newKey && (
        <div className="card-outline rounded-3xl border-2 border-accent/20 bg-accent/10 p-6">
          <h3 className="font-display text-xl text-accent">Your New API Key</h3>
          <p className="mt-1 text-xs text-accent">Copy this key now. It will not be shown again.</p>
          <div className="mt-3 flex items-center gap-3">
            <code className="flex-1 break-all rounded-xl bg-surface-card px-4 py-3 text-xs font-mono text-ink">{newKey}</code>
      ...
      {/* Revoked Banner */}
      {revoked && (
        <div className="card-outline rounded-3xl border border-yellow-500/20 bg-yellow-500/5 p-6">
          <p className="text-sm font-semibold text-yellow-400">API key revoked successfully.</p>
      ...
      {/* Error Banner */}
      {errorMessage && (
        <div className="card-outline rounded-3xl border border-red-500/20 bg-red-500/5 p-6">
          <p className="text-sm font-semibold text-red-400">{errorMessage}</p>
          <p className="text-xs text-red-400">Please try again. If this keeps happening, contact support.</p>
        </div>
      )}
      ...
      {isApproved && (
        <div className="card-outline rounded-3xl bg-surface-card p-6">
          ...
          {apiKey ? (
            <div className="mt-4">
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-accent/10 bg-surface-card px-4 py-3">
                <div>
                  <p className="text-sm font-semibold text-ink">
                    Active key{keyLastFour ? ` ending in ****${keyLastFour}` : ""}
                  </p>
                  {keyCreated && (
                    <p className="text-xs text-ink-muted">Created {keyCreated}</p>
                  )}
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  <form action={revokeKey}>
                    <button className="rounded-full border border-red-500/20 bg-red-500/5 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-red-400">
                      Delete &amp; Revoke key
                    </button>
                  </form>
                </div>
            ...
            <div className="mt-4">
              <p className="text-sm text-ink-muted">No active API key. Click below to generate one.</p>
        ...
        <div className="card-outline rounded-3xl bg-surface-card p-6">
          ...
          <div className="mt-4 space-y-6 text-sm text-ink/80">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-ink-muted">Include your API key in the Authorization header on every request:</p>
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-ink-muted">Returns all available data packages with API pricing.</p>
  ...
  "data": [
    {
      ...
      "price": 5.00
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-ink-muted">Returns your current wallet balance in GHS.</p>
              <p className="mt-2 text-xs font-semibold text-ink">Example cURL</p>
              <pre className="mt-1 overflow-x-auto rounded-xl bg-ink/5 px-4 py-2 text-xs whitespace-pre-wrap break-all">
...
{`{
  ...
  "balance": 150.00
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-ink-muted">Submit a data order. Your wallet is debited immediately.</p>
  ...
  "data": {
    ...
    "createdAt": "2025-01-15T10:30:00.000Z"
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-ink-muted">Returns the current status of an order.</p>
              <p className="mt-2 text-xs font-semibold text-ink">Example cURL</p>
              <pre className="mt-1 overflow-x-auto rounded-xl bg-ink/5 px-4 py-2 text-xs whitespace-pre-wrap break-all">
