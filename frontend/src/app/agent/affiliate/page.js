  ...
  return (
    <div className="space-y-6">
      <div className="glass rounded-3xl p-6">
        ...
        <div className="mt-4 rounded-2xl border border-accent/10 bg-surface-card px-4 py-3 text-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span className="font-semibold text-ink break-all">{referralLink}</span>
            <CopyReferralButton value={referralLink} />
          </div>
          <p className="mt-2 text-xs text-ink-muted">Share this referral URL to grow your network.</p>
        </div>
      </div>
            ...
            downlines.map((item) => (
              <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-accent/10 bg-surface-card px-4 py-3 text-sm">
                <div>
                  ...
                  <p className="text-xs text-ink-muted">Level {item.level}</p>
                </div>
