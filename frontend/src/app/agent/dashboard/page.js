...
export default async function AgentDashboardPage() {
  ...
  const StatIcon = ({ icon }) => {
    switch (icon) {
      ...
      case "zap": return <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-accent/15"><svg className="h-4 w-4 text-accent" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg></div>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="card-outline rounded-2xl bg-surface-card p-5 sm:p-6">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-lg text-ink sm:text-xl">Performance Overview</h2>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 sm:gap-4">
            <div>
              ...
              <p className="mt-1 text-base font-semibold text-ink sm:text-lg">GHS {Number(dashboard?.walletBalanceGhs ?? 0).toFixed(2)}</p>
              <p className="text-[10px] text-accent">↑ 18.2%</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-[0.2em] text-ink-muted">Profit</p>
              <p className="mt-1 text-base font-semibold text-ink sm:text-lg">GHS {Number(dashboard?.profitGhs ?? 0).toFixed(2)}</p>
              <p className="text-[10px] text-accent">↑ 14.7%</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-[0.2em] text-ink-muted">Commissions</p>
              <p className="mt-1 text-base font-semibold text-ink sm:text-lg">GHS {Number(dashboard?.commissionsGhs ?? 0).toFixed(2)}</p>
        ...
        <div className="card-outline rounded-2xl bg-surface-card p-5 sm:p-6">
          ...
          <div className="mt-6 flex flex-col items-center text-center">
            ...
            <h3 className="mt-4 font-display text-lg text-ink sm:text-xl">You're trending above target.</h3>
      ...
      <div className="card-outline rounded-2xl bg-surface-card p-5 sm:p-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="font-display text-lg text-ink sm:text-xl">Recent Orders</h2>
          ...
          {recentOrders.length === 0 ? (
            ...
            <>
              ...
              <div className="sm:hidden space-y-3">
                {recentOrders.slice(0, 5).map((order) => (
                  <div key={order.id} className="rounded-2xl border border-accent/10 bg-surface-elevated p-4">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-ink">#{order.id?.slice(-8) || order.id}</span>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <p className="text-[10px] uppercase tracking-wider text-ink-muted">Customer</p>
                        <p className="mt-0.5 text-ink">{order.customerName || "Customer"}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-wider text-ink-muted">Amount</p>
                        <p className="mt-0.5 font-semibold text-ink">GHS {Number(order.totalAmountGhs || 0).toFixed(2)}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-wider text-ink-muted">Product</p>
                        <p className="mt-0.5 text-ink">{order.productName || "Data Bundle"}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-wider text-ink-muted">Date</p>
                        <p className="mt-0.5 text-ink">{order.createdAt ? new Date(order.createdAt).toLocaleDateString() : "--"}</p>
                      </div>
                    </div>
                  </div>
                ))}
